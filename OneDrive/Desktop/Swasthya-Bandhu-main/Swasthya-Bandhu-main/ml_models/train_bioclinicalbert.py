# ============================================================
# train_bioclinicalbert.py
# Bio_ClinicalBERT + HealthCareMagic-100k + MedQuAD
# Google Drive checkpointing — resume after Colab disconnects
# ============================================================
# COLAB CELL 0 — Run this first, every session:
#
#   !pip install -q transformers==4.40.0 datasets torch scikit-learn tqdm
#
#   from google.colab import drive
#   drive.mount('/content/drive')
#
# Then run all remaining cells top to bottom.
# If session disconnects mid-training, just re-run from Cell 0.
# The script auto-resumes from the last saved checkpoint.
# ============================================================
...
DRIVE_DIR   = "/content/drive/MyDrive/swasthya_training"   # all checkpoints saved here
...
PROCESSED_CSV = os.path.join(DRIVE_DIR, "combined_dataset.csv")
URGENCY_PKL   = os.path.join(DRIVE_DIR, "urgency_encoder.pkl")
SPECIALIST_PKL= os.path.join(DRIVE_DIR, "specialist_encoder.pkl")

if os.path.exists(PROCESSED_CSV):
    # Already processed in a previous session — skip re-downloading
    print("Found cached dataset on Drive — loading...")
    ...
    # ── Source 1: HealthCareMagic-100k ───────────────────────
    # Real patients typing symptoms to doctors — primary source
    ...
    # ── Source 2: MedQuAD ────────────────────────────────────
    # Medical Q&A — adds symptom phrasing variety
    print("\nLoading MedQuAD...")
    try:
        mq = load_dataset("lavita/MedQuAD")["train"]
        print(f"  Rows: {len(mq)}")
        for item in tqdm(mq, desc="MedQuAD"):
            # MedQuAD has 'question' field — treat as symptom description
            text = str(item.get("question", "")).strip()
            if len(text) >= 30:
                records.append(make_record(text))
    except Exception as e:
        print(f"  MedQuAD load failed: {e} — continuing with HealthCareMagic only")
    ...
    print(f"\nUrgency classes    : {list(urgency_enc.classes_)}")
    print(f"Specialist classes : {list(specialist_enc.classes_)}")

    # Save to Drive so next session skips this step
    ...
    print(f"\nSaved dataset + encoders to {DRIVE_DIR}")

# Ensure encoded columns exist (needed when loading from cache)
if "urgency_encoded" not in df.columns:
    df["urgency_encoded"]    = urgency_enc.transform(df["urgency"])
if "specialist_encoded" not in df.columns:
...
print(f"Urgency classes ({NUM_URGENCY})    : {list(urgency_enc.classes_)}")
print(f"Specialist classes ({NUM_SPECIALIST}): {list(specialist_enc.classes_)}")
...
def save_checkpoint(stage, epoch, model, optimizer, best_acc, history):
    """Save full checkpoint to Drive — survives Colab disconnects"""
...
def load_checkpoint(stage, model, optimizer):
    """Returns (start_epoch, best_acc, history) — 0,0.0,[] if no checkpoint"""
    path = os.path.join(DRIVE_DIR, f"ckpt_{stage}.pth")
...
# batch_size=16 fits safely in free Colab T4 (15GB VRAM)
train_loader = DataLoader(train_ds, batch_size=16, shuffle=True,  num_workers=2, pin_memory=True)
val_loader   = DataLoader(val_ds,   batch_size=32, shuffle=False, num_workers=2, pin_memory=True)
print(f"Train batches: {len(train_loader)}  Val batches: {len(val_loader)}")


# ── CELL 8: run_stage — with Drive checkpointing + resume ────
def run_stage(stage, epochs, optimizer, scheduler, loss_fn, patience=3):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if metrics["specialist_acc"] > best_acc:
            ...
            no_improve = 0
            # Save best weights separately for clean loading
            torch.save(model.state_dict(),
                       os.path.join(DRIVE_DIR, f"best_{stage}.pth"))
        ...
        # Checkpoint every epoch — so any disconnect loses at most 1 epoch
...
for p in model.bert.embeddings.parameters(): p.requires_grad = False  # keep embeddings frozen
...
FINAL_PTH = os.path.join(DRIVE_DIR, "biobert_clinical_best.pth")
...
print("Download these to ml_models/models/ on your PC:")
print("  biobert_clinical_best.pth")
print("  urgency_encoder.pkl")
print("  specialist_encoder.pkl")
